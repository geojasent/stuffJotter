...
const deleteLocation = async (req: Request, res: Response) => {
  ...
  try {
    const deleteItemlistLocation = await pool.query(
      `DELETE from itemlist where user_id = '${user}' AND place = '${location}' RETURNING *`
    );
    const deleteStoredfilepathLocation = await pool.query(
      `DELETE from stored_file_path where user_id = '${user}' AND place = '${location}' RETURNING *`
    );
    const deleteUserLocation = await pool.query(
      `DELETE from userplaces where user_id = '${user}' AND place = '${location}' RETURNING *`
    );
