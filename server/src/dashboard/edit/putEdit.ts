...
const getUserLocationItems = async (req: Request, res: Response) => {
  try {
    ...
    const locationItemData = await pool.query(
      `SELECT * from itemlist WHERE user_sub = '${user}' AND place = '${location}'`
    );
...
const putItem = async (req: Request, res: Response) => {
  try {
    ...
    const putItem = await pool.query(
      `UPDATE itemList SET user_sub = $1, place = $2, item = $3 ,item_quantity = $4, item_purchase_price = $5, item_total_price = $6, item_purchase_date = $7, item_description = $8, item_file_path = $9 WHERE user_sub = '${user}' AND item_id = ${itemId} RETURNING *`,
      [
        user,
        location,
        data.item,
...
const putItemFile = async (req: any, res: Response) => {
  try {
    ...
    const postFileUpload = await pool.query(
      `INSERT INTO stored_file_path (item_id, user_sub, place, dashboard, file_path, file_size) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (item_id) DO UPDATE SET item_id = $1, user_sub = $2, place = $3, dashboard = $4, file_path = $5, file_size = $6`,
